...
public class DecryptionCes {
    public static void decryption() throws IOException {
        String alphabetString = "абвгдеёжзийклмнопрстуфхцчшщъыьэюя.,\"\":!? ";
        List<Character> characters = new ArrayList<>();
        String s = "Decryption is done, your file is saved in the src folder.";
        FileWriter out = new FileWriter("src\\fileD.txt");
        ...
        try {
            ...
            while (reader.ready()) {
                ...
                if (alphabetString.indexOf(ch) != -1) {
                    ch = alphabetString.charAt(((((alphabetString.indexOf(ch) - key) % alphabetString.length())) + alphabetString.length()) % alphabetString.length());
